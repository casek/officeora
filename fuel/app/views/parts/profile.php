    <!-- PROFILE SECTION START -->
    <section id="profile" class="section">
        <div class="container section-wrapper">
            <div class="section-content">
                <div class="row">          
                    <div class="col-md-7 match-height">

                        <!-- PROFILE TEXT START -->
                        <div class="profile-text padding-right-yes">
                            <div class="row">
                                <div class="col-md-12">
                                    <h2 class="section-title uppercase">こんにちは。オフィスORAです</h2>

                                    <p>
                                        オフィスORAは六藤圭介（ムトウケイスケ）が20年に渡り運営しているIT系個人事業です。
                                    </p>

                                    <p>
                                        「価値」を生みだす為のITプロジェクトをあらゆる形でサポートします。
                                    </p>

                                    <span class="divider"></span>

                                    <p class="text-grey">
                                        ITプロジェクトの効率化を指向して、アジャイル開発手法やDevOpsを追求していて気付いたことがあります。<br>
                                        ITプロジェクトのすべては「価値を生みつづける為に行われるべきである」ということです。<br>
                                        一見当たり前のように聞こえますが、中小規模のITプロジェクトに関わっていると、このことが忘れられて物事が進むのを目の当たりにすることがほとんどです。<br>
                                        「ビジネスとITプロジェクトの断絶」...<br>
                                        規模は本来関係ありません。中小規模のプロジェクトであっても、大規模プロジェクトで普通に行われていることと同じことができるはずですし、工夫して実施することで大規模プロジェクトから生まれたものとの競争がはじめて可能になるのではないでしょうか？<br>
                                        「コストが...」などと目先のことに追われて重要なポイントが無視・省略されるような状況を避け、ひたすら「価値を生む為」にプロジェクトを進行するには...<br>
                                        その答えを追求しつつ、ITプロジェクトに関わり結果を出しつづける...そんなサービスを顧客に提供したいと考えています。
                                    </p>
                                </div>
                                <!-- //.col-md-12 -->
                            </div>
                            <!-- //.row -->
                        </div>
                        <!-- //PROFILE TEXT END -->

                        <!-- CONTACT DETAILS START -->
                        <div class="contact-details padding-right-yes">
                            <div class="row contact-details-body">
                                <div class="col-sm-6 col-md-5">
                                    <h3 class="uppercase">Contact Details</h3>

                                    <ul class="list-unstyled text-grey">
                                        <li>オフィスORA (Office ORA)</li>
                                        <li>代表 六藤圭介 (Keisuke Mutoh)</li>
                                        <li>東京都中央区佃2-2</li>
                                        <li>kmutoh[at mark]office-ora.com</li>
                                    </ul>
                                </div>
                                <!-- //.col-md-5 -->

                                <div class="col-sm-6 col-md-7">
                                    <a href="assets/resume.pdf" class="btn btn-lg btn-custom">
                                        <i class="icon-File-Download"></i> 業務履歴ダウンロード 
                                    </a>
                                </div>
                                <!-- //.col-md-6 -->
                            </div>
                            <!-- //.row -->
<?php include APPPATH.'views/parts/social.php'; ?>                   
                        </div>
                        <!-- //CONTACT DETAILS END -->

                    </div>
                    <!-- //.col-md-7 -->

                    <!-- PROFILE PICTURE START -->
                    <div class="col-md-5 hidden-xs hidden-sm match-height pp-wrapper">
                        <div class="profile-picture style-one">
                            <img src="assets/images/profile-picture-1.jpg" alt="profile picture" class="img-responsive" style='width:487px; height:810px;'/>
                        </div>
                    </div>
                    <!-- //PROFILE PICTURE END -->
                </div>
                <!-- //.row -->
            </div>
            <!-- //.section-content -->
        </div>
        <!-- //.container -->
    </section>
    <!-- //PROFILE SECTION END -->
